   ...
   try {
       ...
       dueLoans.forEach(async (loan) => {
           ...
           if (user.balance >= loan.amount) {
               ...
               const updatedUser = await User.findByIdAndUpdate(loan.user, { balance }, { new: true });
               const updatedLoan = await loanRequest.findByIdAndUpdate(loan._id, { paid: false }, { new: true });
               ...
               await loanRequest.findByIdAndUpdate(loan._id, { paid: false }, { new: true });
           }
       });
   } catch (error) {
       console.error(error);
